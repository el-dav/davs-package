...
const writeTyp = (typ, relativePath) => {
  typ.write(templatesService.getTypTemplate(relativePath));
};
...
const createAsset = (assetName, path) => {
  ...
  const typesFolderPath = atom.config.get('davs-package.typesPath');
  const typesDir = `${rootDir}${typesFolderPath}/types`;
  const relativePath = utilsService.getRelativePath(assetDir, typesDir);

  const typPath = `${assetDir}/${assetName}.typ.js`;
  const typ = new File(typPath);
  typ.create()
    .then(() => {
      writeTyp(typ, relativePath);
    })
    .then(() => {
