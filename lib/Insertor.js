...
const insertComponent = (fileContents, component) => {
  ...
  ownProps.forEach((prop) => {
    if (prop[0].replace('?', '') === 'children') {
      hasChildren = true;
    }
  });

  ownProps.filter(prop => prop[0].replace('?', '') !== 'children' && prop[0].replace('?', '') !== 'routeParams');
  ...
  text += ` ${hasChildren ? `></${component.name}>` : '/>'}`;
...
class Creator {
  ...
  onInsertItemSelected(component) {
    ...
    /*
    const editor = atom.workspace.getActiveTextEditor();
    editor.insertText(`<${component.name} />`, {
      select: true,
    });
    */

    console.log('component path:', component.path);
