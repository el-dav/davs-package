...
const createDuck = (duckName) => {
  ...
  // with atom file
  /*
  const reducersFile = new File(`${rootPath}${duckFolderPath}/reducers.js`);
  reducersFile.read(true).then((contents) => {
    console.log('contents', contents);
  });
  console.log(`${rootPath}${duckFolderPath}/reducers.js`);
  console.log(fs.readFileSync(`${rootPath}${duckFolderPath}/reducers.js`, 'utf8'));
  */
...
const createAsset = (assetName, path) => {
  ...
  if (!path) {
    ...
    assetDir = `${rootPath}${assetFolderPath}/${assetName}`;
  } else {
