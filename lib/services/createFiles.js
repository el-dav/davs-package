...
const writeType = (type, typeName) => {
  type.write(templatesService.getTypeTemplate(typeName));
};
...
const createType = (typeName) => {
  const rootPath = getRootDir();
  const typeFolderPath = atom.config.get('davs-package.typesPath');
  const typeDir = `${rootPath}${typeFolderPath}/${typeName}`;
  mkdirp.sync(typeDir);

  const typePath = `${typeDir}/${typeName}.js`;
  const type = new File(typePath);
  type.create()
  .then(() => {
    writeType(type, typeName);
  })
  .then(() => {
    atom.workspace.open(typePath);
  });
};
